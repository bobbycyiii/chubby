...
def findNonSeparatingAnnulus(finite_mfld):
  T = finite_mfld
...
def old_isT2xI(regina_mfld):
  ...
  if not mu.isSolidTorus():
    return False
  simplifyCusps(M)
  T = M.getBoundaryComponent(1)
  n = T.getNumberOfEdges()
  assert n == 3
  for i in range(0,n):
    clone = NTriangulation(M)
...
def isHyp(regina_manifold):
  m = NTriangulation(regina_manifold)
  m.intelligentSimplify()
